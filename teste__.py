...
def codigo_formiga(dados__: dict):
    ...
    def cxTwoPointCopy(ind1, ind2):
        """Execute um cruzamento de dois pontos com cópia nos elementos de
        entrada. A cópia é necessária porque o fatiamento em numpy retorna uma
        visão dos dados, o que leva a uma sobrescrição de si mesmo na operação
        de troca. Isso previne
        ::
    ...
    def eaSimpleWithElitism(
        ...
        """Este algoritmo é semelhante ao algoritmo eaSimple() do DEAP, com a modificação de que
        o halloffame é usado para implementar um mecanismo de elitismo. Os indivíduos contidos
        no halloffame são diretamente injetados na próxima geração e não estão sujeitos aos 
